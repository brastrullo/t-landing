...
<div
	...
	on:scroll={onScroll}
	class="overflow-x-hidden bg-transparent overscroll-none 3xl:container 3xl:mx-auto snap-y snap-mandatory"
>
	...
	>
		<div class="flex flex-col justify-end sm:h-auto h-[100dvh] snap-end sm:snap-none introContainer">
			<!-- style:opacity={`${10 / sTop}`} -->
			<p
				style:transform={`translate(-${sTop * 0.7 * 10}px, ${sTop}px)`}
				class={`will-change-scroll z-40 text-red-400 home-main-text text-5xl sm:text-7xl sm:max-w-80 mx-6 sm:mx-4 md:max-w-[40rem] font-bold mb-12 sm:mb-0 5xl:text-8xl`}
				in:fade
			...
			<!-- <span
				...
				class={`mb-8 animate-bounce sm:visible pb-1 sm:hidden text-center text-xs font-extralight opacity-60 z-50`}
				>scroll down</span
			> -->
		</div>
		<div
			class={`buttonContainer h-screen snap-end sm:snap-none sm:visible sm:h-auto text-2xl flex flex-col justify-end sm:w-1/2 sm:bottom-20 sm:right-20 text-center sm:text-right pb-8 sm:pb-0 scroll-pb-8 sm:mb-0`}
		>
